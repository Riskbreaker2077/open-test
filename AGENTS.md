...
**La 031 está implementada (15/09/2026).** La pantalla de proyección muestra un tablero de asistencia: un cuadro por estudiante convocado, con nombre y apellido, blanco si no ha entrado, verde si está conectado, rojo si salió y verde con ✓ si entregó. "Salió" se detecta por silencio: `server/presencia.js` guarda en memoria cuándo se vio por última vez cada intento (lo renueva `conIntento`, es decir, el sondeo de 5 s del examen y de la sala de espera) y lo da por desconectado a los 15 s o al tocar "Salir". `GET /api/docente/proyeccion/:id` devuelve `estudiantes` con solo `nombre`, `curso` y `estado`. Revisa por decisión del docente la regla "nada de nombres" de la 012; se corrigió `mission.md` primero. Además, el tiempo mínimo por pregunta por defecto pasó de 10 a 60 s.
