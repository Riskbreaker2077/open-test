...
24. **024 · [Pausar la evaluación desde el examen del estudiante](../features/024-pausar-desde-examen/spec.md)** — botón "Pausar y salir" en el examen que llama `POST /api/examen/pausar`; el servidor pone la sesión en `pausada` (mismo estado que la pausa del docente, 008), limpia la cookie del estudiante en esa tablet y lo devuelve al portal; idempotente si la sesión ya estaba pausada.
...
32. **031 · [Asistencia en la pantalla de proyección](../features/031-asistencia-en-proyeccion/spec.md)** — la proyección muestra un tablero con un cuadro por estudiante convocado, con nombre y apellido: blanco si no ha entrado, verde si está conectado, rojo si salió ("Pausar y salir", "Salir" o 15 s sin comunicarse) y verde con ✓ si entregó. La conexión se detecta por el sondeo de 5 s de las tablets y vive en memoria (`server/presencia.js`). Revisa por decisión del docente la regla "nada de nombres" de la 012 y de `mission.md`; notas, avance, preguntas y códigos siguen fuera del proyector.
