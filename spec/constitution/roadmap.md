...
19. **019 · [Estadísticas por pregunta y por competencia](../features/019-estadisticas-pregunta-competencia/spec.md)** — nueva pantalla del panel: qué preguntas y competencias falla más el grupo, por una sesión cerrada concreta o acumulado por banco entre todas sus sesiones cerradas, con filtro de curso.
20. **020 · [Gestión manual de estudiantes](../features/020-gestion-manual-estudiantes/spec.md)** — crear y editar estudiantes uno a uno desde el panel, sin pasar por un CSV; complementario a la importación por archivo de la 002. Modal nativo (`<dialog>`), validación reutilizada del importador, sin dependencias nuevas.
21. **021 · [Bloquear entrega con preguntas pendientes](../features/021-bloquear-entrega-con-pendientes/spec.md)** — quita el botón "Terminar la prueba" del examen y endurece `entregarIntento` para que rechace con 409 las entregas `manual` o `ultima_pregunta` cuando quedan preguntas sin responder; los motivos del sistema (`tiempo`, `forzada_docente`) siguen cerrando aunque haya pendientes.
22. **022 · [Eliminar evaluación con resultados ya descargados](../features/022-eliminar-evaluacion-con-resultados/spec.md)** — nueva columna `sesiones.descargado_en` (migración v4); `borrarSesion` rechaza con 409 las evaluaciones cerradas con intentos mientras esa marca sea NULL, y permite borrarlas (en cascada con sus intentos, intento_preguntas y respuestas) una vez descargadas; el botón "Borrar" del panel aparece deshabilitado con texto de ayuda hasta que el docente baja al menos uno de los cuatro archivos de resultados. _Revisada por la 033: ya no se exige descargar antes de borrar._
23. **023 · [Hostname del equipo en la URL y el QR](../features/023-hostname-en-qr/spec.md)** — `urlsDeIntranet` agrega candidatas por nombre del equipo (`http://<hostname>:3000` y `http://<hostname>.local:3000`, con prioridad negativa) cuando `os.hostname()` lo reporta; el QR de la proyección las prefiere sobre las IPv4 y el cartel de arranque avisa al docente del nombre detectado.
24. ~~**024 · [Pausar la evaluación desde el examen del estudiante](../features/024-pausar-desde-examen/spec.md)**~~ — **revertida por la 032.** Agregaba un botón "Pausar y salir" que pausaba la sesión para todo el grupo.
25. **025 · [Exportaciones: Excel rico + ZIP reproducible](../features/025-exporaciones-excel-zip/spec.md)** — reduce las cuatro descargas a dos: `resultados.xlsx` (Resumen + Detalle con las cuatro opciones y la metadata pedagógica + Banco con conteos) y `reproduccion.zip` (JSON `formato_version: 2` con las imágenes referenciadas empaquetadas y un `imagenes_faltantes.txt` cuando alguna falta en disco). Los CSV detalle, CSV resumen y JSON suelto quedan obsoletos (404).
26. **026 · [Grupos de preguntas y campos informativos](../features/026-grupos-de-preguntas/spec.md)** — adopta los tres tipos de grupo del estándar `preguntas-icfes` v1.2.0 (`contexto_compartido`, `banco_opciones` para matching, `texto_con_blancos` para cloze), los campos informativos de v1.1.0 (`grado`, `prueba`, `procedencia`, `verificado`, `fuentes` por pregunta; `procedencia_justificacion` y `justificacion_verificada` por opción) y los campos de v1.2.0+ (`nivel_mcer`, `valor` por pregunta). El validador vendorizado se actualiza a v1.4.0; el esquema gana tabla `grupos` y 16 columnas nuevas (migración v5); la importación filtra preguntas con `{{numero:<id>}}`; el examen del estudiante rinde matching y cloze en una sola pantalla y la calificación suma `valor`. La exportación sube a `formato_version: 3` (`export-resultados-v3.md`). La verificación visual (tablet/proyector/Excel real) queda en cola con el resto de la sesión física.
27. **027 · [Recuperación de la contraseña del docente](../features/027-recuperar-contrasena/spec.md)** — restablecimiento de la contraseña del panel desde la consola del propio equipo (`OpenTest.exe --recuperar-contrasena`): pide la nueva dos veces con escritura oculta, reutiliza el hashing de la 011, no toca ningún dato y exige acceso físico al equipo — sin enlaces ni avisos en la interfaz web, tal como dejó fijado la 011.
28. **Validación final en equipo destino** — sesión física única en Windows con el aula real: QR y legibilidad en proyector, corte de red, usabilidad táctil, línea gráfica, apertura del `.xlsx` en Excel/LibreOffice, clic-a-clic de la pantalla de estadísticas (019), botones nuevos de la 021/022, el recorrido completo de los tres tipos de grupo de la 026 con `ejemplos/banco-grupos-ingles.zip` (lectura compartida, matching y cloze, más el aviso de exclusión `{{numero:...}}`), y el diálogo de recuperación de la 027 con `OpenTest.exe --recuperar-contrasena` (máscara de escritura y contraseña nueva aceptada al reabrir). **Completada el 14/09/2026, todo el checklist verificado sin novedades.**
29. **028 · [Ingreso manual de preguntas](../features/028-ingreso-manual-preguntas/spec.md)** — en Bancos de preguntas, "+ Nuevo banco vacío" crea un banco sin preguntas y "+ Agregar pregunta" en su detalle abre un formulario con la forma simple original de la 003 (contexto opcional, imagen opcional, enunciado, 4 opciones, cuál es correcta, justificación opcional por opción), sin la metadata pedagógica ni los grupos del estándar — eso sigue siendo exclusivo del ZIP. Cada pregunta suelta gana botones Editar/Eliminar; una pregunta ya usada en una evaluación no se puede tocar (409, mismo principio que 009/022). Complementaria a la carga por ZIP, igual que la 020 lo es de la 002.
30. **029 · [Instalador de Windows](../features/029-instalador-windows/spec.md)** — `npm run build:installer` empaqueta la salida de `build:exe` (010) con Inno Setup en un `OpenTest-Setup-<versión>.exe`: asistente sin permisos de administrador, accesos directos y desinstalador en "Agregar o quitar programas". Excluye `data\` del paquete a propósito para que instalar encima de una copia con evaluaciones cargadas no las borre. La copia manual de la carpeta (010) sigue funcionando igual. **Compilado y probado el 15/09/2026 en el runner de Windows de GitHub Actions (030).**
31. **030 · [Descarga pública del instalador](../features/030-descarga-instalador/spec.md)** — `.github/workflows/instalador.yml` compila `OpenTest.exe` y `OpenTest-Setup.exe` en un runner de Windows y los prueba de verdad: instalar, arrancar (HTTP 200), reinstalar con datos y desinstalar. Con cada tag `vX.Y.Z` publica la release y guarda una copia en `instalador/OpenTest-Setup.exe`. El sitio, el README y la guía enlazan `releases/latest/download/OpenTest-Setup.exe`. Primera versión publicada: **1.0.0** (15/09/2026).
32. **031 · [Asistencia en la pantalla de proyección](../features/031-asistencia-en-proyeccion/spec.md)** — la proyección muestra un tablero con un cuadro por estudiante convocado, con nombre y apellido: blanco si no ha entrado, verde si está conectado, rojo si salió ("Salir" o 15 s sin comunicarse) y verde con ✓ si entregó. La conexión se detecta por el sondeo de 5 s de las tablets y vive en memoria (`server/presencia.js`). Revisa por decisión del docente la regla "nada de nombres" de la 012 y de `mission.md`; notas, avance, preguntas y códigos siguen fuera del proyector.
33. **032 · [Quitar "Pausar y salir" del examen](../features/032-quitar-pausar-y-salir/spec.md)** — revierte la 024: el estudiante ya no tiene el botón y `POST /api/examen/pausar` deja de existir (404), porque pausaba la evaluación para todo el grupo. Pausar vuelve a ser solo del docente.
34. **033 · [Ajustes de usabilidad del aula](../features/033-ajustes-usabilidad/spec.md)** — la proyección gana **Volver** (sale sin tocar la prueba) y "Cerrar" pasa a **Finalizar**; con más de 30 convocados el tablero usa primer nombre y primer apellido en cuadros de una línea; se puede borrar una evaluación sin haber descargado sus resultados (revisa la 022); el estudiante sale de la pantalla de resultados con **Volver al inicio**, que cierra su sesión en la tablet.
...
40. **039 · [Una sola opción con aspecto de elegida](../features/039-una-sola-opcion-elegida/spec.md)** — el resaltado por encima de una opción era idéntico al de "elegida" y en las tablets se quedaba pegado, así que parecía haber dos opciones marcadas. Ahora el hover solo existe con ratón y es distinto, y la elegida lleva ✓. Solo CSS. Queda la comprobación en tablet real.
41. **040 · [Resultado con puntos y pergamino](../features/040-resultado-con-puntos/spec.md)** — en `aciertos` y `completo` el resultado es una rejilla de puntos numerados (verde, rojo, gris); cada punto abre la pregunta en una pantalla con diseño de pergamino y botón Volver. Con puntaje perfecto, en cualquier nivel, el puntaje sale en letras doradas animadas, los puntos se vuelven dorados y aparece un mensaje de felicitación al azar que queda fijo para cada estudiante. Convive con la anulación de la 038. Sin cambios en la API.
...
Ninguna. El roadmap del encargo original quedó completo con la validación física del 14/09/2026; lo posterior (028–040) son mejoras pedidas tras usarlo en el aula. Pendiente en tablet real: confirmar la 039 (una sola opción marcada) y recorrer la 040 con imágenes, tablas y un grupo de emparejamiento. Lo que sigue, si el usuario lo decide, sale de `Backlog / ideas`.
...
| El estudiante puede terminar la prueba con preguntas pendientes | 021 (se eliminó como camino válido; sólo queda la salida legítima: responder todas o esperar el reloj/docente) |
